...
class BuzonUsuarioExternoController extends Controller
{

    public function index(){

        return View::make('usuario_externo.carpetas');
    }
    ...
    public function ver_documento($id, Request $request)
    {
        ...
        ->get(env('API_SGD_DOCUMENTO','http://sgd_ms_documentos:3333').'/api/sgd-documentos/ver');
    ...
    public function generar_archivo_pdf(Request $request)
    {
        ...
        if (file_exists(storage_path('app/public/files/') . $nNombreArchivoCargar))
        {
            ...
            $datosDocumento = Http::withHeaders(['key'=>$sesion_key,'Content-Type'=>'application/json']) //
            ->timeout(30)        
            ->put(env('API_SGD_DOCUMENTO','http://sgd_ms_documentos:3333').'/api/sgd-documentos/generar_archivo', [            
